...
export default function Disciplinas({params}:{params:{id:string}}){
 ...
 async function load(client=s){
  ...
  const [{data:a},{data:b}]=await Promise.all([
   client.from('subjects').select('*').eq('active',true).order('name'),
   client.from('class_subjects').select('subject_id,weekly_lessons,subjects(name)').eq('class_id',params.id)
  ])
  setSubjects(a||[]);setAssigned(b||[])
 }
 useEffect(()=>{const client=createClient();setS(client);load(client)},[])
 async function submit(e:FormEvent){e.preventDefault();if(!s)return;setMsg('');const {error}=await s.rpc('assign_subject_to_class',{p_class_id:params.id,p_subject_id:subjectId,p_teacher_id:null,p_weekly_lessons:weekly});setMsg(error?'Não foi possível adicionar.':'Disciplina adicionada.');if(!error)load()}
